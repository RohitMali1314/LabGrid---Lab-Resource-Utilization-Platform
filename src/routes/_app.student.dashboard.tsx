...
function StudentDashboard() {
  ...
  const eqById = new Map((equipment.data ?? []).map((e) => [e.equipmentId, e]));
                ...
                {available.slice(0, 6).map((e) => (
                  <div key={e.id} className="flex items-center gap-3 rounded-lg border p-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-sm font-medium">{e.equipmentName || e.name}</div>
                      <div className="truncate text-xs text-muted-foreground">
                        {[e.modelNo && `Model ${e.modelNo}`, e.serialNo && `SN ${e.serialNo}`].filter(Boolean).join(" · ")}
                      </div>
                    </div>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <EquipmentImage
                          ...
                          equipmentName={eqById.get(b.equipmentId)?.equipmentName}
                          serialNo={eqById.get(b.equipmentId)?.serialNo}
                        />
                        <span className="truncate">
                          {b.equipmentName || eqById.get(b.equipmentId)?.equipmentName || `#${b.equipmentId}`}
                        </span>
                      </div>
                    </TableCell>
                    <TableCell>{fmtDate(b.startTime)}</TableCell>
