...
function NewBookingDialog({ onCreated }: { onCreated: () => void }) {
  ...
  const selectedEquipment = useMemo(
    () => departmentEquipment.find((e) => String(e.equipmentId) === equipmentId),
    [departmentEquipment, equipmentId],
  );
          ...
          <div className="space-y-1">
            ...
            {selectedEquipment && (
              <div className="mt-2 overflow-hidden rounded-xl border bg-card">
                ...
                />
                <div className="p-3">
                  <div className="text-sm font-medium">{selectedEquipment.equipmentName}</div>
                  <div className="text-xs text-muted-foreground">
                    {selectedEquipment.modelNo ? `Model ${selectedEquipment.modelNo} · ` : ""}
                    {selectedEquipment.serialNo ? `SN ${selectedEquipment.serialNo} · ` : ""}
...
function BookingsPage() {
  ...
  const equipmentById = useMemo(
    () => new Map((equipment.data ?? []).map((e) => [e.equipmentId, e])),
    [equipment.data],
  );
                      ...
                      <TableCell>
                        <div className="flex items-center gap-3">
                          <EquipmentImage
                            ...
                            equipmentName={equipmentById.get(b.equipmentId)?.equipmentName}
                            serialNo={equipmentById.get(b.equipmentId)?.serialNo}
                          />
                          <div className="min-w-0">
                            <div className="truncate font-medium">
                              {equipmentNames.get(b.equipmentId) ?? `#${b.equipmentId}`}
                            </div>
                            <div className="truncate text-xs text-muted-foreground">
                              {equipmentById.get(b.equipmentId)?.modelNo ?? `ID ${b.equipmentId}`}
                            </div>
                          </div>
                        </div>
                      </TableCell>
