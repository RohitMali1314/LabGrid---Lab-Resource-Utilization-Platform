...
import { listBookingsForRole, type Booking } from "@/services/bookingService";
...
function UtilizationPage() {
  ...
  const summary = useApi<LiveUtilizationSummary>(
    useCallback(() => getLiveUtilization(role), [role]),
    [role],
  );
  const equipment = useApi<Equipment[]>(listEquipment, []);
  // Students / researchers only ever see their own bookings — never call GET /api/bookings.
  const bookings = useApi<Booking[]>(
    useCallback(() => listBookingsForRole(role), [role]),
    [role],
  );
  ...
  for (const e of equipList) {
    const key = e.department || lookups.departmentName(e.departmentId);
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <p className="truncate text-sm font-medium">
                        {b.equipmentName || lookups.equipmentName(b.equipmentId)}
                      </p>
