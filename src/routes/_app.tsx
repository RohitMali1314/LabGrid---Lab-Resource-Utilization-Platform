...
import { can, ROUTE_CAPABILITY } from "@/lib/permissions";
...
function AppLayout() {
  ...
  useEffect(() => {
    ...
    const capMatch = ROUTE_CAPABILITY.find((r) => pathname.startsWith(r.prefix));
    if (capMatch && !can(user.role, capMatch.capability)) {
      navigate({ to: ROLE_HOME[user.role], replace: true });
    }
