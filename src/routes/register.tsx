...
/**
 * roleId values from the backend `roles` table. The backend has no /api/roles
 * endpoint, so the mapping is declared here and always submitted as a number.
 */
const REGISTER_ROLES: { roleId: number; label: string }[] = [
  { roleId: 1, label: "Researcher" },
  { roleId: 2, label: "Lab Technician" },
  { roleId: 3, label: "Lab Manager" },
  { roleId: 4, label: "Department Head" },
  { roleId: 5, label: "Institution Admin" },
  { roleId: 6, label: "System Admin" },
  { roleId: 7, label: "Student" },
];
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <SearchableSelect
            ...
            options={REGISTER_ROLES.map((r) => ({
              value: String(r.roleId),
              label: `${r.roleId} — ${r.label}`,
            }))}
          />
          <p className="text-[10px] text-muted-foreground">
            The numeric <code>roleId</code> from the backend <code>roles</code> table is submitted.
          </p>
        </div>
