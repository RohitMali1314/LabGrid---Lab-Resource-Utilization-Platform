...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const loginWithGoogle: AuthCtx["loginWithGoogle"] = async (credential) => {
    ...
    if (!res.token) throw new Error(res.message || "Google login failed");
    // Only the LabGrid JWT is persisted — never the raw Google credential.
    localStorage.setItem(TOKEN_KEY, res.token);
    setToken(res.token);
    const profile = await authService.me();
    const u = fromCurrentUser(profile);
  ...
  return (
    <Ctx.Provider value={{ user, token, login, loginWithGoogle, register, logout, loading }}>
