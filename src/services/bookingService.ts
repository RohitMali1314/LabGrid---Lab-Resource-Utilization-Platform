 ...
 * STUDENT / RESEARCHER  -> GET /api/bookings/my-bookings
 * Management roles      -> GET /api/bookings
 * Any other role        -> no request at all (never call a forbidden endpoint).
 */
export const listBookingsForRole = (role: Role | undefined | null): Promise<Booking[]> => {
  if (can(role, "bookings.viewAll")) return listBookings();
  if (can(role, "bookings.mine")) return listMyBookings();
